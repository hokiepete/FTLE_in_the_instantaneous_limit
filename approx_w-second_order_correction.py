...
yout=np.empty([dimx*dimy,2])
for k,y0 in enumerate(zip(x.ravel(),y.ravel())):
    print(k)
    sol = sint.solve_ivp(vel_func,[tf,t0],y0,rtol=1e-8,atol=1e-8)
    yout[k,:] = sol.y[:,-1]
...
sig = np.ma.empty([dimy,dimx,dimt])
for t in range(dimt):
    for i in range(dimy):
        for j in range(dimx):
            if (dudxdt[i,j,t] and dudydt[i,j,t] and dvdxdt[i,j,t] and dvdydt[i,j,t]) is not np.ma.masked:    
                ...
                s1[i,j,t] = eigenValues[idx[0]]
                s2[i,j,t] = eigenValues[idx[1]]
                
                #s1[i,j,t] = np.min(np.linalg.eig(S)[0])
                #s2[i,j,t] = np.max(np.linalg.eig(S)[0])
                #b1[i,j,t] = np.min(np.linalg.eig(B)[0])
                #b2[i,j,t] = np.max(np.linalg.eig(B)[0])
                sig[i,j,t] = -s1[i,j,t] - abs(tf-t0)*(-s1[i,j,t]**2+0.5*np.dot(np.dot(eigenMin,B),eigenMin))
            else:
...
plt.pcolormesh(x,y,sig)#,levels=np.linspace(sigma.min(axis=None),sigma.max(axis=None),301))
plt.colorbar()
#pu,pv = vel_func(0,[x[::3,::3],y[::3,::3]])
#plt.quiver(x[::3,::3],y[::3,::3],pu,pv)
plt.title('$\\sigma$ approximation, T = {:}'.format(abs(tf-t0)))
...
plt.subplot(224)
#plt.contourf(x,y,sigma,levels=np.linspace(sigma.min(axis=None),sigma.max(axis=None),301))
plt.pcolormesh(x,y,sigma)#,levels=np.linspace(sigma.min(axis=None),sigma.max(axis=None),301))
plt.colorbar()
#pu,pv = vel_func(0,[x[::3,::3],y[::3,::3]])
#plt.quiver(x[::3,::3],y[::3,::3],pu,pv)
plt.title('$\\sigma$')

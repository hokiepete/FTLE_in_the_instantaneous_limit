...
u = 3.6*mf.unstagger(vars['U'][:,height_level,:,:],axis=2)
v = 3.6*mf.unstagger(vars['V'][:,height_level,:,:],axis=1)
lat = vars['XLAT'][0,:,:]
lon = vars['XLONG'][0,:,:]
root.close()
...
sigb = np.ma.empty([tdim,ydim,xdim])
sigf = np.ma.empty([tdim,ydim,xdim])
t0 = 0
tf = 1
for t in range(tdim):
    for i in range(ydim):
        for j in range(xdim):
            if (dudxdt[t,i,j] and dudydt[t,i,j] and dvdxdt[t,i,j] and dvdydt[t,i,j]) is not np.ma.masked:    
                ...
                sigb[t,i,j] = np.ma.masked
                sigf[t,i,j] = np.ma.masked
...
plt.subplot(222)
#cs=m.contourf(lon,lat,sigb,levels=np.linspace(sigb.min(axis=None),sigb.max(axis=None),301),latlon=True)
cs=m.pcolormesh(lon,lat,sigb,latlon=True)
#cs=m.contourf(lon,lat,s1,levels=np.linspace(s1.min(axis=None),s1.max(axis=None),301),latlon=True,vmin=0.5*s1.min(axis=None),vmax=0.5*s1.max(axis=None))
m.drawcoastlines()
m.drawstates()
parallels = np.arange(round(lat_min,0),lat_max+2,2)
...
cs=m.pcolormesh(lon,lat,s2,latlon=True)
#cs=m.contourf(lon,lat,s1,levels=np.linspace(s1.min(axis=None),s1.max(axis=None),301),latlon=True,vmin=0.5*s1.min(axis=None),vmax=0.5*s1.max(axis=None))
m.drawcoastlines()
m.drawstates()
parallels = np.arange(round(lat_min,0),lat_max+2,2)
...
cs=m.pcolormesh(lon,lat,sigf,latlon=True)
#cs=m.contourf(lon,lat,s1,levels=np.linspace(s1.min(axis=None),s1.max(axis=None),301),latlon=True,vmin=0.5*s1.min(axis=None),vmax=0.5*s1.max(axis=None))
m.drawcoastlines()
m.drawstates()
parallels = np.arange(round(lat_min,0),lat_max+2,2)

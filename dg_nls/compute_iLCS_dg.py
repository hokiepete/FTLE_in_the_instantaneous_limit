...
size = 100
x = np.linspace(0,2,2*size)
dx = x[1]-x[0]
y = np.linspace(0,1,size)
dy = y[1]-y[0]
x,y=np.meshgrid(x,y)
u,v = double_gyre(0,[x,y])
...
plt.figure(figsize=[1.5*(5+3/8),0.75*(5+3/8)]) 
attracting_ilcs = plt.contour(x,y,s1_directional_derivative,levels=[0],colors='blue',linewidths=4)
s2_directional_derivative = np.ma.masked_where(abs(x-1)>0.1,s2_directional_derivative)
...
plt.savefig('dg_iles_v1.png', transparent=False, bbox_inches='tight',dpi=300)
plt.savefig('dg_iles_v1.eps', transparent=False, bbox_inches='tight')

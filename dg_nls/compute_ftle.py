...
from velocity_fields import double_gyre, nonlinear_saddle

#integrate velocity field
for k,y0 in enumerate(zip(x.ravel(),y.ravel())):
    sol = sint.solve_ivp(nonlinear_saddle,backward_time,y0,rtol=1e-8,atol=1e-8)
    flow_map[k,:] = sol.y[:,-1]
...
with open(f'ftle_{tf}.pickle','wb') as file:
...
#pu,pv = nonlinear_saddle(0,[x[::3,::3].ravel(),y[::3,::3].ravel()])
